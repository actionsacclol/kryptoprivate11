  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        'Reward rates (the Robinhood Chain and BNB wallet pages): api.merkl.xyz. Listing the published reward campaigns on a chain sends no address. Asking what YOUR wallets are earning necessarily sends that wallet address to Merkl, so the software only does it when you ask it to, and says so where you ask.',
        ...
        'Links you click deliberately may open your browser at solscan.io, robinhoodchain.blockscout.com, bscscan.com, dexscreener.com, x.com, pump.fun, four.meme, ponslaunchpad.com, a token’s own website, dashboard.helius.dev, dashboard.alchemy.com, or our own site.',
        'The Links panel on the Widgets page, only if you add it: a token’s own X, website and launchpad pages are shown inside the software in a separate, sandboxed browser view with its own cookie jar, kept on this machine. Those sites receive the same requests your browser would — your IP address, and any cookies you accept or sign-ins you make there — and nothing from the software: no wallet address, no key, no page of ours. A link is whatever the token’s creator published, so the site behind it is theirs, not ours; treat it as you would any site you had never seen.',
        'Telegram and domain records, when you open a token that links them, or a script you run asks about that token: the software fetches the public preview page of the token’s Telegram link from t.me — the title and the member count Telegram shows anyone — and the public registration record of the token’s website domain over RDAP from the registry for that domain ending, found through the list at data.iana.org: registration date, expiry, registrar. These requests go to Telegram, to IANA and to the registry, never to the token’s own website, and carry nothing of yours beyond your IP address. At most sixty of each an hour. What the token’s website itself says (whether it names the token, which accounts it links) is read only from the page you opened in the Links panel, never fetched.',
