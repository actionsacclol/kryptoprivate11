...
export function parseMintExtensions(data: Uint8Array): MintExtensions | null {
  ...
  if (d.length <= ACCOUNT_TYPE_OFFSET) return d.length === MINT_BASE_LEN ? { permanentDelegate: null, nonTransferable: false, transferHook: null, name: null, symbol: null } : null;
  if (d[ACCOUNT_TYPE_OFFSET] !== ACCOUNT_TYPE_MINT) return null;
  const out: MintExtensions = { permanentDelegate: null, nonTransferable: false, transferHook: null, name: null, symbol: null };
