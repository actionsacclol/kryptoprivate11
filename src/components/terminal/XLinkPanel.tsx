  ...
  return (
    <div className="rounded-lg border border-white/10 bg-black/25 px-3 py-2.5">
      <div className="mb-2 flex items-center gap-2">
        ...
        <span className="font-display text-micro uppercase tracking-heading text-krypt-muted">X link</span>
        ...
        {twitter && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1 text-label text-krypt-muted transition hover:text-white"
          >
      ...
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
        <span className={cls('text-note font-semibold', isAccount ? 'text-white' : 'text-arc-gold')}>{link.label}</span>
        {!isAccount && (
      ...
      {(reuse.handle > 0 || reuse.post > 0) && (
        <div className={cls('mt-2 rounded-md px-2.5 py-2 text-body leading-relaxed', farmed ? 'bg-rose-500/10 text-rose-200' : 'bg-white/[0.03] text-krypt-muted')}>
          {reuse.post > 0 && (
            <div>
      ...
      {isAccount && (
        <details className="mt-2">
          <summary className="cursor-pointer text-label text-krypt-muted/70 hover:text-white">
            What to check on the account itself
          </summary>
          <ul className="mt-1.5 space-y-1">
            {CHECKLIST.map((line) => (
              <li key={line} className="flex gap-2 text-label leading-relaxed text-krypt-muted/80">
                <span className="mt-[6px] h-1 w-1 flex-shrink-0 rotate-45 bg-krypt-purple/60" />
                <span>{line}</span>
              </li>
            ))}
          </ul>
          <p className="mt-1.5 text-label leading-relaxed text-krypt-muted/50">
