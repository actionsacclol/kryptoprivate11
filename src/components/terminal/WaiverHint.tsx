// "Hold N $KRYPTO and this fee halves" — the one line, in one place.
//
...
//
// Rules it keeps:
//   • It never appears while the fee is already halved — there is nothing to
...
import { KRYPTO_HOLDER_TOKENS, kryptoTokenLive } from '@shared/krypto';
...
export function WaiverHint({
  ...
  const { t } = useLocale();
  if (halved || !charged || !kryptoTokenLive()) return null;
  return (
    <span className={cls('font-semibold text-amber-300', className)}>
      ...
      {t('waiver.hold', { amount: KRYPTO_HOLDER_TOKENS.toLocaleString() })}
    </span>
  );
}
